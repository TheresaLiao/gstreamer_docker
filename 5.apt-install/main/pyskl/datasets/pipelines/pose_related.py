...
class PoseDecode:
    def __call__(self, results):
        ...
        # print("frame_indsframe_indsframe_indsframe_indsframe_indsframe_indsframe_indsframe_indsframe_indsframe_indsframe_indsframe_inds",results['frame_inds'])
...
class PreNormalize2D:
    ...
    def __call__(self, results):
        ...
        keypoint =  results['keypoint'].astype(np.float32)
...
class MergeSkeFeat:
    ...
    def __call__(self, results):  # results is a dict
        feats = []
        for name in self.feat_list:
            feat = results.pop(name) # shape : (1,40,17,3)
            feats.append(feat)

        feats = np.concatenate(feats, axis=self.axis) # (1,frameNum, 17,3)
...
class GenSkeFeat:
    ...
    def __call__(self, results):
        ...
        #     print("GENNNNNNNNNNNNNNNNNNNNNN")
...
class FormatGCNInput:
    ...
    def __call__(self, results):
        ...
        M, T, V, C = keypoint.shape
        # print(" M, T, V, C M, T, V, C M, T, V, C", M, T, V, C)
        nc = results.get('num_clips', 1) # nc =  10
        assert T % nc == 0
